...
<div class="container-fluid">
    <div class="row py-2">
        ...
        <div class="col-xl-4 col-md-4 col-12">
            <a href="{{route('role.create')}}" class="theme-btn rounded-2 float-end"><i class="bi bi-plus-lg" aria-hidden="true"></i> CREATE</a>
        </div>
    </div>
    @if(session('success'))
                            ...
                            <td class="text-center">
                                <div class="form-check form-switch">
                                    <input class="form-check-input status-change pointer " role="button"
                                        type="checkbox" data-id="{{$role->id}}" data-active="{{$role->active}}"
                                        {{ $role->active ? 'checked' : '' }}>
                                </div>
                            </td>
                        </tr>
    ...
    (()=>{
        ...
        $(document).on('click', '.status-change', function(e) {
            ...
            Swal.fire({
                title: 'Change status?',
                text: active == 1 ?
                    'Do you want to deactivate this role?' : 'Do you want to activate this role?',
                icon: 'question',
